...
/** 
 *   将虚拟地址[va, va+size)与物理地址[pa, pa+size)建立映射关系
 *   Overview:
...
extern struct sig_list free_sig_list;
extern struct tf_list free_tf_list;
...
int env_alloc(struct Env **new, u_int parent_id) {
	...
	sig_init_env(e->env_id, parent_id);
...
void env_run(struct Env *e) {
	...
	/*if (curenv -> sa_handler != NULL) {
		//printk("%x\n", e->env_tf.cp0_epc);
		struct Trapframe tf = e -> env_tf;
		tf.regs[31] = e->sig_return_addr;
		tf.regs[4] = 4;
		curenv -> sa_handler = NULL;
		env_pop_tf(&tf, e->env_asid);	
	}
	env_pop_tf(&(e->env_tf), e->env_asid);	*/
	struct LinkedTf *tf;
	tf_alloc(&tf);
	tf->tf=e->env_tf;
	LIST_INSERT_HEAD(&curenv->tf_stack, tf, tf_link);
	do_signal(0);
}

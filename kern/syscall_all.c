...
int sys_sigprocmask(int how, const sigset_t *set, sigset_t *oldset) {
	return modify_proc_mask(how, set, oldset);
}

int sys_sigaction(int signum, const struct sigaction *act, struct sigaction *oldact) {
	return sig_register(signum, act, oldact);
}

int sys_kill(u_int envid, int sig) {
	return sig_send(envid, sig);
}
...
void sys_sig_init(void* returnAddr) {
	sig_set_return_addr(returnAddr);
}
void *syscall_table[MAX_SYSNO] = {
	...
	[SYS_kill] = sys_kill,
	[SYS_sig_return] = sys_sig_return,
	[SYS_sig_init] = sys_sig_init,
};

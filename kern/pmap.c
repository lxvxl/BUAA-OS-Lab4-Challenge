...
/** 
 *   查找虚拟地址va对应的页表项。如果对应的页表尚未建立，则根据create判断是否需要建立页表
 * 	 Overview:
...
static int pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte) {
		...
		if(create == 1) {
			if(page_alloc(&pp) == 0) {
				*pgdir_entryp = page2pa(pp);
				pp->pp_ref++;
				*pgdir_entryp = PTE_V|PTE_D|(*pgdir_entryp);
			} else {
				return -E_NO_MEM;
			}
		} else {
	...
	pgtab = (Pte*)KADDR(PTE_ADDR(*pgdir_entryp));
	*ppte = pgtab + PTX(va);
	return 0;
}

/** 
 *	 将虚拟地址va与页面pp建立映射。如果va已经与另一个物理页面形成映射，则删除这个映射。
 * Overview:
